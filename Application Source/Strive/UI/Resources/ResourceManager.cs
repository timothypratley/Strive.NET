...
namespace Strive.UI.Resources
{
	...
	public class ResourceManager
	{
		...
		public static Model LoadModel(int modelID)
		{
			// check MDL first:
			
			if(System.IO.File.Exists(System.IO.Path.Combine(_modelPath, modelID.ToString() + ".mdl")))
			{
				return Model.Load(modelID.ToString(), System.IO.Path.Combine(_modelPath, modelID.ToString() + ".mdl"), ModelFormat.MDL);
			}
			else if (System.IO.File.Exists(System.IO.Path.Combine(_modelPath, modelID.ToString() + ".3ds")))
			{
				return Model.Load(modelID.ToString(), System.IO.Path.Combine(_modelPath, modelID.ToString() + ".3ds"), ModelFormat._3DS);
			}
			else
			{
