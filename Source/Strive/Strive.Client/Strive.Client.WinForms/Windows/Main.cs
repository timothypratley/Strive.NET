...
{
	public class Main : Strive.Client.WinForms.Windows.SystemFormBase
	{
		...
		private void loadSettings()
		{
			if(System.IO.File.Exists(Settings.SettingsManager.MagicWindowSettingsPath))
			{
				...
				Log.Info("Loaded Magic Window config from '" + Settings.SettingsManager.MagicWindowSettingsPath + "'");
			}
		...
		private void saveSettings()
		{
			...
			Log.Info("Saved Magic Window config to '" + Settings.SettingsManager.MagicWindowSettingsPath + "'");
			Settings.SettingsManager.SaveWindowSetting(this);			
			Settings.SettingsManager.PersistStriveSettings();
			Log.Info("Saved Strive config to '" + Settings.SettingsManager.StriveSettingsPath + "'");
		}
		...
		private void RenderTarget_Click( object sender, System.EventArgs e ) {
			...
			if ( Game.GameControlMode ) 
			{
				...
				Game.CurrentServerConnection.Send(
					new Strive.Network.Messages.ToServer.UseSkill(
						Game.CurrentGameCommand, 0, targets )
				);
				Log.Info( "Using skill " + Game.CurrentGameCommand + " on " + m.Name );
                 */
			//} else {
				//SetGameControlMode();
			//}
...
//			DockingManager.AddContentWithState(connectionWindow, State.DockLeft);
		...
		private void ViewMiniMap_Click(object sender, System.EventArgs e) {
			if ( miniMap == null ) {
				...
				Log.Error( "Only one minimap allowed." );
				return;
			}
			//miniMap.Show();
