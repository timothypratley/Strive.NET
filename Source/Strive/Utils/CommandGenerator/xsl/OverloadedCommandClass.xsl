<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/" >
		...
		public class CommandCache
		{
			...
			/// <summary>
			/// Finalisers the associated Connection when the CommandCache is finalised
			/// </summary>
			~CommandCache()
			{
				_connection.Close();
	...
	<xsl:for-each select="//StoredProcedure">
		...
		<xsl:call-template name="HeaderSimple">
			<xsl:with-param name="StoredProcedure" select="."/>
		</xsl:call-template>
	...
	<xsl:template name="Header">
		<xsl:param name="StoredProcedure" />
			...
			public System.Data.SqlClient.SqlCommand <xsl:value-of select="$StoredProcedure/@name" />(<xsl:call-template name="HeaderParams">
																										<xsl:with-param name="Parameters" select="$StoredProcedure/Parameters" />
																									</xsl:call-template>)
			{
				...
				<xsl:call-template name="SetParameters"><xsl:with-param name="Parameters" select="./Parameters" /></xsl:call-template>
	...
	<xsl:template name="SetParameters">
		...
		<xsl:for-each select="$Parameters/Parameter">
					thisCommand.Parameters["<xsl:value-of select="./@name" />"].Value = <xsl:value-of select="substring-after(./@name,'@')" />;
		</xsl:for-each>
