...
%>
		<td class="Normal" align="right"><a href="<%=www.strive3d.net.Utils.ApplicationPath%>/screenshots/<%=System.IO.Path.GetFileName(s)%>"><img border="0" height="200" width="200" src="<%=www.strive3d.net.Utils.ApplicationPath%>/DesktopModules/Strive/Thumbnailer.aspx?i=<%=downloadsFolder.Replace("~", www.strive3d.net.Utils.ApplicationPath)%>/<%=System.IO.Path.GetFileName(s)%>&amp;h=200&amp;w=200" /></a><br /><%=created.ToString("dd MMM yyyy h:mm tt")%></td>
<%
	count++;
}	
%>
